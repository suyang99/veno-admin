<?php

namespace plus\src;

use App\Plus\src\Http\Middleware\AfterInjectDcatPlus;
use App\Plus\src\Http\Middleware\BeforeInjectDcatPlus;
use App\Plus\src\Http\Middleware\MiddleInjectDcatPlus;
use Dcat\Admin\Extend\ServiceProvider as BaseServiceProvider;

/**
 * Class ServiceProvider
 * @package Celaraze\DcatPlus
 */
class ServiceProvider extends BaseServiceProvider
{
    protected $js = [
        'js/index.js',
    ];
    protected $css = [
        'css/index.css',
    ];
    protected $middleware = [
        'before' => [
            BeforeInjectDcatPlus::class,
        ],
        'middle' => [
            MiddleInjectDcatPlus::class,
        ],
        'after' => [
            AfterInjectDcatPlus::class,
        ]
    ];
    protected $menu = [
        [
            'title' => 'Dcat Plus',
            'uri' => 'dcat-plus/site',
            'icon' => 'feather icon-settings'
        ]
    ];

    public function register()
    {
        //
    }

    public function settingForm()
    {
        return new Setting($this);
    }

    public function init()
    {
        parent::init();

    }
}
