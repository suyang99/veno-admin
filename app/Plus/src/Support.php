...
namespace App\Plus\src;


use App\Plus\src\Extensions\Form\SelectCreate;
use App\Models\Config;
use App\Plus\src\Extensions\Show\Video;
...
CSS
    ...
    public static function updateConfigData($input, $type)
    {
        $webKey = Config::query()->where('type', $type)->pluck('key', 'id')->toArray();
        try {
            DB::beginTransaction();
            foreach ($input as $index => $v) {
                if (in_array($index, $webKey)) {
                    Config::query()->where('id', array_keys($webKey, $index))->update(['key' => $index, 'value' => $v]);
                }
            }
            DB::commit();
